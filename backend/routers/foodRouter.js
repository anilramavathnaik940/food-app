...
import Category from "../models/foodModel.js";
...
foodrouter.get("/all", async (req, res) => {
  try {
    const foodItems = await Category.find(); // Fetch all food items
    res.status(200).json({data:foodItems, message:"Sucessful"});
  } catch (error) {
...
foodrouter.post("/add", async (req, res) => {
    try {
      ...
      if (existingCategory) {
        ...
        return res.status(200).json({ message: 'Items added to existing category', data: existingCategory });
      }
      else {
        // Create a new category with items
        const newCategory = new Category({ category, items });
        await newCategory.save();
        return res.status(201).json({ message: 'New category added successfully', data: newCategory });
      }
    } 
    catch (error) {
      console.error('Error in POST /api/food/add:', error);
...
foodrouter.put("/update", async(req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Food data updated successfully', result });

  } catch (error) {
   res.status(500).json({ message: 'Error updating food data', error }); 
  }
});

foodrouter.delete('/del/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await Category.findByIdAndDelete(id);
    res.status(200).json({ message: 'Food data deleted successfully', result });
    
  } catch (error) {
    res.status(500).json({ message: 'Error deleting food data', error });
  }
});
...
foodrouter.delete("/delete", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "All food items deleted", result });
  } catch (error) {
    console.error("Error deleting food items:", error.message);
